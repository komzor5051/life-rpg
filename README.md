...
В строке меню: `LVL 3 · 41 500 HP`, после 11:00 без касания добавляется «касание?». По клику открывается панель `public/menu.html` (SwiftBar 2 показывает её как web view вместо меню): герой, уровень, опыт, стрик, босс недели, main quest, чек-лист дня и кнопки «Продал» (сумма, кому, пресеты), «Касание +1», «Публикация +1», «Закрыть день», «Открыть кабинет». На SwiftBar 1.x web view нет, там будет обычное меню с карточкой-картинкой из `menubar/render_card.py` (нужен Pillow). Всё пишется в `Журнал.md`, кабинет подхватывает сам: он опрашивает сервер каждые 15 секунд и при возврате во вкладку.
